...
Use the below command to sync the contents of a folder. This is different than copying because it will check what files are already present and only uploads new or changed files (without a log). The syncing function `rsync` operates on contents rather than folders so the destination path is slightly different than when using `cp` *NOTE the `-d` flag will also delete files on the cloud that no longer exist on the local drive.*
